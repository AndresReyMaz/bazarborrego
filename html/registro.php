...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($_POST['matricula'])) {
        $errors[] = 'Olvidaste meter tu matricula.';
    } else {
    ...
    if (empty($_POST['nombre'])) {
        $errors[] = 'Por favor escribe tu numbre.';
    } else {
    ...
    if (empty($_POST['apellido'])) {
        $errors[] = 'Por favor escribe tu apellido.';
    } else {
    ...
    if (!empty($_POST['pass1'])) {
        if ($_POST['pass1'] != $_POST['pass2']) {
            $errors[] = 'Tus contraseñas no coinciden.';
        } else {
            $p = mysqli_real_escape_string($dbc, trim($_POST['pass1']));
        }
    } else {
        $errors[] = 'Por favor escribe una contraseña.';
    }
